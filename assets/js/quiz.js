$(function () {
    ...
    $("body").on("click", ".respostas button.bto", function () {
        ...
        // Verificando se idSlide e userOption são válidos
        if (perguntas[idSlide] && perguntas[idSlide].respostas[userOption]) {
            ...
            feedback(idSlide, validacao, function () {
                pontuar(validacao);
            });
        } else {
            console.error("Invalid idSlide or userOption:", idSlide, userOption);
        }
...
let templateQuiz = function (i) {
    ...
    // Adicionando a imagem da pergunta se existir
    if (perguntas[i].pergunta.imagem) {
        let img = perguntas[i].pergunta.imagem;
        let imagemHtml = '<img src="' + img.src + '" alt="' + img.alt + '" title="' + img.title + '" class="img-fluid" />';
        txt.append(imagemHtml);
    }
    ...
    for (a = 0; a < nro_respostas; a++) {
        ...
        if (resposta.imagem) {
            respostas.addClass("row");
            let img = resposta.imagem;
            imagemHtml = '<img src="' + img.src + '" alt="' + img.alt + '" title="' + img.title + '" class="img-fluid resposta-imagem" />';
        }

        respostas.append('<button data-resp="' + a + '" type="button" data-label="' + resposta.ordem + '" class="bto img-selecionada">' + imagemHtml + resposta.texto + '</button>' + '<p class="legenda">');
    }
...
let templateDragInDrop = function (i) {
    ...
    alertInstrucao.addClass('alert alert-primary align-items-center alert-draggin');
    ...
    msgContainer.append('<img src="../assets/img/ico_arrastar.png" alt="clique e arraste">');
